...
- HC-05 蓝牙串口接收遥控命令。
- 蓝牙命令、EC11 和本地按键改变状态后立即回传 `status=...`，并每 5 秒自动推送一次当前状态，方便手机端和串口助手确认链路。
...
状态回传格式示例。播放、停止、上下曲、音量、静音、播放顺序、测试音和直接选曲命令执行后会立即追加 `status=...`：
...
- 模拟端到端演示流程：APK 按钮、HC-05 命令、固件回包、手机面板变化
