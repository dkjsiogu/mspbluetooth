"""Generate the course-design report draft for the environment monitor."""
...
def firmware_version() -> str:
    ...
    match = re.search(r'ENV_FIRMWARE_VERSION\s+"([^"]+)"', text)
    if not match:
        raise RuntimeError("ENV_FIRMWARE_VERSION not found")
    return match.group(1)


def build_report() -> str:
    version = firmware_version()
    return f"""# 单片机技术课程设计报告：多点温度/环境监测系统
...
本项目形成了传感器采集、OLED 显示、蓝牙通信、内部 Flash 历史、蜂鸣器报警和编码器阈值调整的完整软件框架。代码按应用、驱动、通信、存储和验证工具分层，并通过自动化脚本保留可重复的构建与仿真证据，便于课程验收和后续上板调试。
"""
