...
DEMO_STEPS = [
    ...
    DemoStep("Trace", "x", "firmware reports the recent Bluetooth/local control event ring"),
...
def run_demo() -> list[DemoSnapshot]:
    ...
    if "bt:trace" not in android.trace_text or "bt:tone" not in android.trace_text:
        raise AssertionError(f"final trace panel mismatch: {android.trace_text!r}")
...
def main() -> int:
    ...
    print(snapshots[-1].trace.replace("\n", " | "))
